   ...
   // $("#txtEditor2").val($("#txtEditor").Editor("getText"));
    ...
    <body>
        ...
        <div class="header">
            ...
            <div class="hed">  
                <h1>Update  <span style="color:red"> product </span></h1>
            </div>
            @if (count($errors) > 0)
...
@endif
    <div class="portfolio-box-caption">
        <div class="portfolio-box-caption-content">
            <form action="{{route('img_en.delete',$te)}}" method="get">

                {!! csrf_field() !!}
                <input type="hidden"  name="post_id" value="{{$product->id}}"/>
             
                <button type="submit" class="btn btn-danger"  name="submit{{$i}}">delete</button>
            </form>
         </div>
        </div>
      </div>
...
@endforeach
                <form id="order" class="contactform style4 clearfix" method="post" 
                      action="{{route('orders_en.new_product.update',$product->id)}}" 
